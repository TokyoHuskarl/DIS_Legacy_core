...
// if you use v[__id($metavar)] in array operation, TPC compiler errors and says "Specified cast is not valid"
// so if you want to use array operation with meta variables, you must write like this:
// `$metavar1.copy $metavar2, 2` -TkHuskarl
...
(cont.) Character string variable (file output)<br>
    ...
    //Basically files will be exported at "Gamedirectory/Text/a.txt", so if you want to export somewhere out of Text directory, set "a" string like this:.toFile(../{your_directory_path}/a)
    //You cannot export files outside Game directory.
    //Also, the default encoding seems sjis so you should set .utf8 option everytime you use file system operands unless you're japanese - TkHuskarl 
