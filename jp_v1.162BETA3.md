...
### 総論
  ...
  - 攻城兵器の変更  
   ...
   - バランス調整  
	   - 帝国以外の人間ファクションの一般戦闘ユニットの生産速度を一律で上昇。  
...
### 各論
  ...
  - 武器の変更  
    - 「ハンドゴン」の増加ADを104->112へ変更。  
		  改めて使ってみたら弱すぎたため。  
    - 「コンポジットボウ」の見た目を試験的に差し替え（やしおじ様からのご厚意による）。  
...
#### UI/UXの変更
  ...
  - ドラッグ時の範囲表示レスポンスをより素早く変更。  
	...
	- マウスカーソルがユニットと重なっている時、クリックできるユニットが強調して表示されるように変更。  
	  ...
	  作者は気にしていませんでしたが、この機能、実はかなり必要だった説があります…  
...
おそらくv1.161の半分程度の軽さになりました。  
  ...
  - 経路探索を最適化。  
    - ゲーム仕様上、6個までしかウェイポイントを保持できず、しかも6⃣個通過するごとに再検索していたDISの経路探索でしたが、  
    js制御の導入によってデータが保持されるように。  
    大規模戦等の検索回数が減るはず。  
  - 戦闘ダメージ計算処理を高速化。  
...
#### その他
  - 前版で廃止されていた敵の戦闘検知処理をロールバック。串良のＮＰＣ異常挙動の原因がこれを廃止したことの可能性があるため。関係なかったらまた切りま  す
