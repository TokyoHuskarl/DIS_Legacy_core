...
### ゲーム総論
  ...
  - 武器パラメータの調整  
    - 両手武器の威力を全体的にほんの少しだけ減少。  
      - 上記の過密状態を減らす変更により近接ユニットが敵に密着する頻度が減ったため、一回の攻撃機会にダメージを出せて少しだけ射程の長い両手武器が相対的に強くなりました。  
        そこで、両手持ち武器全体にかかっている威力調整式の値を本当にほんの少しだけ下方修正しました。  
        とはいえ効果は殆ど体感できないはず。      
...
おそらくv1.161の半分程度の軽さになりました。  
  ...
  - 士気計算処理も高速化。  
  - 建築物によるユニット検索処理も個別ユニット同様の方式で行うように変更。  
  - その他細かい計算を効率化。  
...
- 別個の建物において、同一の研究を重複して研究していると、研究完了時にクラッシュやデータ破損といった重大な誤作動が生じる不具合を修正。  
- Escapeキーを連打しながらゲームを開始すると操作できなメニュー表示が出て詰んでいた不具合を修正。  
#### ユニットの不具合
- ルーリキアにおいて時代進化後に増設したTCで「属州総督の追放」が行えなかった不具合を修正。  
- 同様に増設TCで「荷車」の研究が行えなかった不具合を修正。  
...
- 建物が既に倒したユニットと内部IDが同一の味方ユニットを攻撃することがあったのを修正。  
- 防衛建築物のAIが上手く動いてくれないことがあった不具合を修正。  
#### 計算周り
